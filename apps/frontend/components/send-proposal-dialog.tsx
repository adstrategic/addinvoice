...
export function SendProposalDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      setSubject(`Proposal ${proposalNumber}`);
      setMessage(
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await apiClient.post(
        `/proposals/${proposalSequence}/send`,
        {
          email: email.trim(),
      ...
      if (proposalSequence != null) {
        queryClient.invalidateQueries({
          queryKey: proposalKeys.detail(proposalSequence),
        });
        queryClient.invalidateQueries({ queryKey: proposalKeys.lists() });
      }
      toast.success("Proposal is being sent", {
        description: isAccepted
          ? "The proposal will be sent shortly."
          : `Proposal has been sent to ${email}`,
      });
      setTimeout(() => {
        setSent(false);
        onOpenChange(false);
        if (proposalSequence != null) {
          router.push(`/proposals/${proposalSequence}`);
        }
      }, 2000);
    } catch (error: unknown) {
      setSending(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose how you want to send this proposal to {clientName}
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs defaultValue="email" className="mt-4">
          ...
          <TabsContent value="email" className="space-y-4 mt-4">
            ...
            <div>
              <Label>Subject</Label>
              <Input
                placeholder="Proposal from ADSTRATEGIC"
                className="mt-1"
                value={subject}
