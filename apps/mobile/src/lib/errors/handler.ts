...
import type { ApiErrorCode, UnifiedErrorResponse, ValidatorErrorItem } from '@/lib/api/types'

/**
 * Normalised error class used across the app.
 * `code` matches the backend error codes (see ApiErrorCode for known codes).
...
export function isValidatorErrorArray(data: unknown): data is ValidatorErrorItem[] {
	return (
		Array.isArray(data) &&
		data.length > 0 &&
		data.every(
...
function parseValidatorErrorArrayToFields(data: ValidatorErrorItem[]): Record<string, string[]> {
 ...
 * Handles: client ZodError, network errors, validator middleware array,
 * unified backend contract, and generic HTTP errors.
 */
export function handleApiError(error: unknown): never {
	// Client-side Zod parse failure (response didn't match expected schema)
	if (error instanceof ZodError) {
		throw new ApiError(500, 'INTERNAL_ERROR', 'Received invalid data from server', error.issues)
	}
	...
	if (error instanceof AxiosError && error.response) {
		...
		throw new ApiError(status, `HTTP_${status}`, `Request failed with status ${status}`, data)
	}
	...
	throw new ApiError(500, 'INTERNAL_ERROR', 'An unexpected error occurred', error)
}
